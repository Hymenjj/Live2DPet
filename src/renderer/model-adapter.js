...
class Live2DAdapter extends ModelAdapter {
    ...
    async load(container) {
        ...
        // Auto-resize window to match model aspect ratio
        this._autoResizeWindow();
    ...
    _fitModel() {
        ...
        // Fit model within window — use the smaller scale to avoid clipping
    ...
    /**
     * Auto-resize the pet window to match the model's aspect ratio.
     * Called once after model loads. Keeps width, adjusts height.
     */
    async _autoResizeWindow() {
        ...
        try {
            if (window.electronAPI && window.electronAPI.setWindowSize) {
                await window.electronAPI.setWindowSize(finalW, finalH);
                console.log(`[Live2DAdapter] Auto-resized window to ${finalW}x${finalH} (model aspect: ${aspect.toFixed(2)})`);
                // Re-fit after resize
                setTimeout(() => this._fitModel(), 100);
            }
        } catch (e) {
            console.warn('[Live2DAdapter] Auto-resize failed:', e);
        }
    }
